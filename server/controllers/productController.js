...
exports.getProductById = async (req, res) => {
    try {
        ...
        const images = await Image.find({ product_id: product._id, is_deleted: false });
        ...
        if (productWithDetails.length > 0) {
            ...
            res.status(200).json({ success: true, data: productWithDetails[0] });
        } else {
            return res.status(404).json({ success: false, message: 'Không thể tìm thấy thông tin sản phẩm' });
        }
    } catch (error) {
...
exports.createProduct = async (req, res) => {
    try {
        upload.array('images', 5)(req, res, async (err) => {
            if (err) {
                // Xử lý lỗi nếu có
                return res.status(500).json({ success: false, message: 'Lỗi khi xử lý tệp tải lên' });
            }
            ...
            if (existingProduct) {
                if (existingProduct.is_deleted) {
                    ...
                    return res.status(200).json({ success: true, message: 'Sản phẩm đã được khôi phục thành công' });
                } else {
            ...
            return res.status(200).json({ success: true, message: 'Sản phẩm đã được tạo thành công' });
        });
...
exports.editProduct = async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, message: 'Lỗi khi chỉnh sửa sản phẩm', error: error.message });
    }
};
...
exports.deleteProduct = async (req, res) => {
    try {
        ...
        // Xóa sản phẩm
        const product = await Product.findOneAndUpdate({ _id: product_id }, { is_deleted: true, updated_at: Date.now() });
...
exports.getProductsBySubcategory = async (req, res) => {
    try {
        ...
        const subcategory = await Subcategory.findOne({ subcategory_name, is_deleted: false });
        ...
        const products = await Product.find({ subcategory_id: subcategory._id, is_deleted: false }).sort(sortOptions);
        return res.status(200).json({ success: true, data: products });
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Lỗi khi lấy danh sách sản phẩm theo subcategory_id', error: error.message });
    }
};
...
exports.getProductsByCategory = async (req, res) => {
    try {
        ...
        const category = await Category.findOne({ product_name: categoryName, is_deleted: false });
        ...
        const products = await Product.find({ subcategory_id: { $in: subcategoryIds }, is_deleted: false }).sort(sortOptions);

        return res.status(200).json({ success: true, data: products });
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Lỗi khi lấy danh sách sản phẩm theo category_id', error: error.message });
    }
};
...
exports.getNewestProducts = async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, message: 'Lỗi khi lấy sản phẩm mới nhất', error: error.message });
    }
};
...
exports.search = async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ success: false, message: 'Lỗi khi tìm kiếm sản phẩm', error: error.message });
    }
}
...
exports.checkUserHasPurchased = async (req, res) => {
    try {
        ...
        const orderItems = await OrderItem.find({ product_id: product_id, is_deleted: false });
        ...
        const orders = await Order.find({ _id: { $in: orderIds }, is_deleted: false });
...
exports.isProductFavorite = async (req, res) => {
    try {
        ...
        const favorite = await Favorite.find({ product_id: product_id, user_id: userId, is_deleted: false });
